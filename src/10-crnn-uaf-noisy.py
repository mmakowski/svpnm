...
runs = 50
train_example_counts = [1000, 5000, 10000, 30000, 60000]
test_examples = 20000
# -- early stopping:
...
def _train_and_evaluate(train_file: str, test_file: str, train_examples: int, random_state: int) -> dict:
    ...
    x_train, y_train, _ = _format_data(datasets['train'], max_length)
    x_test, y_test, id_test = _format_data(datasets['test'], max_length)
    ...
    eval_result = _evaluate(model, x_test, y_test, id_test)
    log.info("test accuracy: %f", eval_result['accuracy'])
    return dict(train=fit_result.history, test=eval_result, vocabulary_size=vocab_size)
...
def _create_model(embed_dim: int,
    ...
    model = keras.models.Sequential([
        ...
        keras.layers.Conv1D(filters=conv_filters,
                            kernel_size=conv_kernel_size,
                            padding=conv_padding,
                            activation='relu',
                            strides=conv_strides),
        keras.layers.MaxPooling1D(pool_size=conv_pool_size),
